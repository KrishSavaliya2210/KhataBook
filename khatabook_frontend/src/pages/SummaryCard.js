...
import { getTransactionSummary } from "../api"; // your axios call
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        // Map transactions and compute positive/negative amounts
        ...
        <BarChart data={data}>
          ...
          <Bar dataKey="amount">
            {data.map((entry, index) => (
              <Cell
                key={`cell-${index}`}
                fill={entry.amount >= 0 ? "#28a745" : "#dc3545"} // green for credit, red for debit
              />
            ))}
          </Bar>
        </BarChart>
